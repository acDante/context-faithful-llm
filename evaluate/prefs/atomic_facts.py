...
from prefs.lm import QwenModel, LlamaModel
...
class AtomicFactGenerator(object):
    def __init__(self, model_name=None, cache_dir_prefix='.'):
        try:
        ...
        # Choose which model to use as atomic fact generator
        if "gpt" in model_name:
            self.lm = OpenAIModel(model_name=model_name)
        elif "Qwen" in model_name:
            self.lm = QwenModel(model_name=model_name)
        elif "Llama" in model_name:
            self.lm = LlamaModel(model_name=model_name)
...
# TODO: Store the filtered atomic facts in the cache