...
def mint(x_tok, y_tok):
    ...
    lcs_ = lcs(' '.join(x_tok), ' '.join(y_tok))
    lcsr = lcs_ / len(y_tok)
    mint = (
        1.0
        if lcsr == 0.0
...
def process_batch(nlp, batch):
    ...
    # return [faster_mint(s, t) for s, t in zip(s_tokens, t_tokens)]
...
def faster_mint(x_tok, y_tok, max_lcs_length=2000):
    """Fast and safe MINT calculation"""
    overlaps = ngram_overlaps(x_tok, y_tok)
    
    # Truncate for LCS if too long, but scale the result
    if len(x_tok) > max_lcs_length or len(y_tok) > max_lcs_length:
        x_truncated = x_tok[:max_lcs_length]
        y_truncated = y_tok[:max_lcs_length]
        matcher = SequenceMatcher(None, x_truncated, y_truncated)
        lcs_truncated = sum(triple.size for triple in matcher.get_matching_blocks())
        # Scale back proportionally
        lcs_ = lcs_truncated * (len(y_tok) / len(y_truncated))
    else:
        matcher = SequenceMatcher(None, x_tok, y_tok)
        lcs_ = sum(triple.size for triple in matcher.get_matching_blocks())
    ...
    d.update({"lcsr": (lcs_, len(y_tok), lcsr), "mint": mint_score})
    return d